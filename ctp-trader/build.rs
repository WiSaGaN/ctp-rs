fn main() {
    ...
    let so_symlink_string = format!("{}/libthosttraderapi.so", out_dir);
    ...
    std::os::unix::fs::symlink(&format!("{}/../api-ctp/lib/thosttraderapi.so", current_dir), so_symlink).expect("failed to create new symlink");
